...
HttpData::HttpData(EventLoop *loop, int connfd):
        loop_(loop),
        channel_(new Channel(loop, connfd)), // new一个新的Channel对象，包装连接描述符
        fd_(connfd),
        error_(false),
        ...
        keepAlive_(false)
{
    ...
    channel_->setReadHandler(bind(&HttpData::handleRead, this)); // 已连接描述符的读事件
    channel_->setWriteHandler(bind(&HttpData::handleWrite, this)); // 已连接描述符的写事件
    channel_->setConnHandler(bind(&HttpData::handleConn, this)); // 已连接描述符的连接事件
}
...
// 分离定时器，把原本绑定在HttpData上的旧的定时器分离，并将其设置为删除的
...
// 核心函数2：写入http响应，将输出缓冲写入socket fd 
...
{
    // 分离定时器，该socket上发生了事件，就要更新定时器时间，做法是：把旧的定时器丢弃，重新绑定新的定时器
    seperateTimer();
    __uint32_t &events_ = channel_->getEvents();

	// 设置感兴趣的events事件
    if (!error_ && connectionState_ == H_CONNECTED) // 旧的已经建立连接的fd
    {
        if (events_ != 0) 
        {
            ...
            if ((events_ & EPOLLIN) && (events_ & EPOLLOUT)) // 非新建的连接，就监听可写事件
            {
        ...
        else if (keepAlive_) // 新到来的连接，就监听可读事件
        {
        ...
        else // 新到来的连接，且不是
        {
            ...
            events_ |= (EPOLLIN | EPOLLET);
            //events_ |= (EPOLLIN | EPOLLET | EPOLLONESHOT);
            int timeout = (DEFAULT_KEEP_ALIVE_TIME >> 1); // 为什么不用DEFAULT_EXPIRED_TIME
            loop_->updatePoller(channel_, timeout);
        }
    }
...
AnalysisState HttpData::analysisRequest()
{
    ...
    else if (method_ == METHOD_GET || method_ == METHOD_HEAD)
    {
		...
		// 解析请求文件的类型，供头部字段"Content-Type:"使用
